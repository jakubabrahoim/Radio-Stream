...
function AudioPlayer(database: any) {
    ...
    useEffect(() => {
        ...
        if (currentRadioStation.stationThumbnail !== '')
        ...
        if (
            ...
            currentRadioStation.autoPlay !== false
        ) {
            initialPlayStream();
        ...
        let getFirestoreDoc = async (uid: string) => {
            ...
            let document = await getDoc(
                doc(database.database, 'liked-stations', documentId)
            );
            if (document.exists()) {
    ...
    */
    ...
    async function likeStation(): Promise<void> {
        if (verified === false) {
            console.log(
                'You must verify your email address before you can like a station.'
            );
            return;
        }
        ...
        <div className='grid grid-cols-6 sm:gap-10 justify-items-center items-center fixed bottom-0 w-screen h-20 bg-gray-100 z-50'>
            ...
            <div className='col-span-4 sm:col-span-2 flex flex-row items-center w-auto'>
                <div className='w-14 h-14 bg-gray-300 rounded-lg mr-6 flex items-center justify-center'>
                    {thumbnailPresent ? (
                        <img
                            src={currentRadioStation.stationThumbnail}
                            alt='radio thumbnail'
                            className='w-6 h-6'
                        />
                    ) : (
                        <IconContext.Provider
                            value={{ className: 'text-gray-500 w-6 h-6' }}
                        >
                            <BiRadio />
                        </IconContext.Provider>
                    )}
            ...
            <div className='col-span-2 justify-self-end mr-8 sm:mr-0 sm:justify-self-auto'>
                <button
                    data-e2e='audio-player-play-button'
                    id='playButton'
                    onClick={
                        currentRadioStation.autoPlay === true
                            ? playStream
                    ...
                    className={`rounded-full border border-gray-800 bg-gray-800 hover:bg-gray-700 w-12 h-12 flex items-center justify-center 
                ...
                >
                    {audioPlaying === 'stopped' && (
                        <IconContext.Provider
                            value={{ className: 'text-white pl-0.5 w-6 h-6' }}
                        >
                            <BsPlayFill data-e2e='audio-player-paused-icon' />
                        </IconContext.Provider>
                    )}
                    {audioPlaying === 'playing' && (
                        ...
                        >
                            <BsStopFill data-e2e='audio-player-playing-icon' />
                        </IconContext.Provider>
                    )}
                    {audioPlaying === 'loading' && (
                        <IconContext.Provider
                        ...
                        >
                            <AiOutlineLoading data-e2e='audio-player-loading-icon' />
                        </IconContext.Provider>
                    )}
                    <span id='playButtonLabel' hidden>
                        Play/Stop
                    </span>
                </button>
            </div>
            ...
            <div className='hidden sm:col-span-2 sm:flex flex-row items-center'>
                <div className={`${user !== null ? 'mr-10' : ''}`}>
                    <div className='flex flex-row items-center'>
                        <p className='text-sm mr-14 invisible sm:visible'>
                            Volume
                        </p>
                        <button
                    ...
                    <input
                        ...
                        className={`w-full hover:cursor-grab accent-gray-800 ${
                            currentRadioStation.streamUrl === '' &&
                ...
                {user !== null && verified === true && (
                    <>
                        ...
                        >
                            ...
                            <span id='likeButtonLabel' hidden>
                                Like button
                            </span>
                        </button>
                    </>
                ...
                {user !== null && verified === false && (
                    ...
                    >
                        <button
                            onClick={likeStation}
                            className='rounded-full hover:cursor-not-allowed w-10 h-10 flex items-center justify-center'
                        >
                            {stationLiked === false && (
                                <IconContext.Provider
                                    value={{
