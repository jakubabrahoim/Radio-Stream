    ...
    */
    ...
    useEffect(() => {
        let getLikedStations = async (uid: string) => {
            let firestoreQuery: any = query(
                collection(database.database, 'liked-stations'),
                where('uid', '==', uid)
            );
    ...
    async function removeFromLikedStations(stationName: string): Promise<void> {
        try {
            ...
            setLikedStationsBase(
                likedStationsBase.filter(
                    (station: Station) => station.stationName !== stationName
                )
            );
            /* We don't set this from baseLikedStations because there might a specific search that is currently displayed */
            setLikedStations(
                likedStations.filter(
                    (station: any) => station.stationName !== stationName
                )
            );
        } catch (error) {
            console.log(error);
    ...
    function searchStations(event: any): void {
        ...
        setLikedStations(
            likedStationsBase.filter((station: Station) =>
                station.stationName
                    .toLowerCase()
    ...
    return (
        <>
            ...
            {/* Liked stations */}
            <article
                data-e2e='liked-stations-container'
                className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-7 justify-items-center h-[400px] sm:h-[450px] overflow-y-auto mt-2 mb-6 lg:mb-0'
            >
                {likedStations
                    ...
                    .map((station, id) => {
                        return (
                            ...
                            >
                                ...
                                <div
                                    data-e2e={`liked-station-logo-${id}`}
                                    className='row-span-6'
                                >
                                    {station.stationThumbnail !== '' ? (
                                        <img
                                            src={station.stationThumbnail}
                                            alt='station icon'
                                            className='w-20'
                                        ></img>
                                    ) : (
                                ...
                                <div className='flex flex-row items-center mb-2 w-full'>
                                    <div className='flex basis-2/3 justify-center'>
                                        <button
                                            data-e2e={`liked-station-play-button-${id}`}
                                            className='
                                                justify-self-center
                                    ...
                                    <div className='flex basis-1/3 justify-center'>
                                        <button
                                            data-e2e={`liked-station-remove-button-${id}`}
                                        ...
                                        >
                                            <IconContext.Provider
                                                value={{
                                                    className:
                                                        'text-gray-800 w-7 h-7',
                                                }}
                                            >
                                                <TiDeleteOutline />
                                            </IconContext.Provider>
                                            <span id='removeFromLiked' hidden>
